...
exports.update = async (req, res) => {
  ...
  try {
    if (title === '') req.body.title = 'Không có tiêu đề'
    if (description === '') req.body.description = 'Thêm mô tả ở đây'
    const currentBoard = await Board.findById(boardId)
    if (!currentBoard) return res.status(404).json('Không tìm thấy bảng')
